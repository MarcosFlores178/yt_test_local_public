        ...
        with yt_dlp.YoutubeDL(ydl_opts_list) as ydl:
            ...
            if 'entries' in channel_info:
                for entry in channel_info['entries']:
                    ...
                    # Filtro inteligente de tu código anterior
                    if "PROGRAMADO" in title or "ESPERA" in title or estado == 'is_upcoming':
                        ts = entry.get('release_timestamp')
                        if ts: proximos_ts.append(ts)
                        continue # Salta al siguiente video de los 20

                    # Si llegamos acá, puede ser un vivo real
                    v_id = entry.get('id')
