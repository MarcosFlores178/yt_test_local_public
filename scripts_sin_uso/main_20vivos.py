...
def get_youtube_link(channel_url, cache):
    ...
    ydl_opts_list = {
        ...
        'no_warnings': True, 
        'extract_flat': True,
        'playlist_items': '1-20', # <--- Aumentamos a 20 para saltar los programados
        'cookiefile': COOKIE_FILE,
    }

    try:
        with yt_dlp.YoutubeDL(ydl_opts_list) as ydl:
            print(f" (Buscando entre 20 videos...) ", end="")
            channel_info = ydl.extract_info(search_url, download=False)
            
            if 'entries' in channel_info:
                # Filtramos: primero los que tengan más probabilidad de ser vivo
                for entry in channel_info['entries']:
                    # Si el título dice "PREMIERE" o "PRÓXIMAMENTE", ni perdemos tiempo
                    title = entry.get('title', '').upper()
                    if "PROGRAMADO" in title or "ESPERA" in title:
                        continue
                    ...
                    try:
                        with yt_dlp.YoutubeDL(ydl_opts_check) as ydl_check:
                            ...
                            # La prueba definitiva: is_live tiene que ser True
                            if info.get('is_live'):
                                link = info.get('url')
                                cache[channel_url] = {'link': link, 'timestamp': time.time()}
                                return link
                    except:
                        continue 
        return None
