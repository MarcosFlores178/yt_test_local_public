...
def get_youtube_link(url):
    ydl_opts = {
        'quiet': True,
        'no_warnings': True,
        'format': 'best', # Esto sirve de base
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # Buscamos en la lista de formatos disponibles
            formats = info.get('formats', [])
            
            # Intentamos encontrar el manifest_url que es el "HLS Variant"
            # Este es el link que suele funcionar en cualquier IP
            manifest_url = info.get('manifest_url')
            if manifest_url:
                return manifest_url
            
            # Si no está el manifest_url, buscamos el formato que sea m3u8 y tenga 'hls_variant'
            for f in formats:
                if 'manifest_url' in f:
                    return f['manifest_url']
                if f.get('protocol') == 'm3u8_native' or 'm3u8' in f.get('url', ''):
                    # Preferimos los que dicen "playlist/hls_variant"
                    if 'hls_variant' in f['url']:
                        return f['url']
            
            # Si nada de lo anterior funciona, devolvemos el URL básico
            return info.get('url')
