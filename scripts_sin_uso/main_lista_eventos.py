...
def get_youtube_data(channel_url, cache, original_name):
    """
    Busca todas las señales en vivo y el próximo evento.
    Retorna una lista de diccionarios con las señales encontradas.
    """
    # 1. Intentar usar el caché (si es reciente y tiene links)
    if channel_url in cache:
        data = cache[channel_url]
        if data.get('vivos') and (time.time() - data['timestamp'] < 7200):
            # Verificamos que al menos la primera señal siga online
            if is_link_online(data['vivos'][0]['link']):
                return data['vivos'], data.get('next_event')

    # 2. Configuración de búsqueda profunda en la pestaña /streams
    ...
    ydl_opts_list = {
        'quiet': True, 
        'no_warnings': True, 
        'extract_flat': True, # No entra al video, solo lee la lista
        'playlist_items': '1-10', 
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts_list) as ydl:
            print(f" (Analizando señales...) ", end="")
            ...
            if 'entries' in channel_info:
                ...
                for entry in channel_info['entries']:
                    ...
                    # CASO A: Es un directo activo
                    if estado == 'is_live' or entry.get('is_live'):
                        video_id = entry.get('id')
                        # Obtenemos el link real del m3u8 para este video específico
                        real_link = extract_m3u8(f"https://www.youtube.com/watch?v={video_id}")
                        if real_link:
                            vivos_encontrados.append({
                                'name': f"{original_name} - Señal {contador_senal}",
                                'link': real_link
                            })
                            contador_senal += 1

                    # CASO B: Es un evento programado
                    elif estado == 'is_upcoming':
                        ts = entry.get('release_timestamp')
                        if ts: proximos_ts.append(ts)

            # Guardamos en caché lo encontrado
            proximo_inicio = min(proximos_ts) if proximos_ts else None
            cache[channel_url] = {
                'vivos': vivos_encontrados,
                'timestamp': time.time(),
                'next_event': proximo_inicio
            }
            
            return vivos_encontrados, proximo_inicio
