...
def is_link_online(url):
    ...
    try:
        ...
        # 2. PRUEBA REINA: ¿El manifiesto dice que terminó?
        # Leemos los primeros 5000 bytes (suficiente para ver el final de un m3u8)
        content = r.iter_lines()
        line_count = 0
        for line in content:
            line_str = line.decode('utf-8', errors='ignore').upper()
            if "#EXT-X-ENDLIST" in line_str:
                # Si aparece esto, es un video grabado/finalizado
                return False
            line_count += 1
            if line_count > 100: break # No leemos de más
