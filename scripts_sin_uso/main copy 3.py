...
#     """Verifica si un link IPTV estático responde (HEAD request)."""
#     try:
...
# def is_link_online(url):
#     """Verifica si un link responde simulando ser un navegador."""
#     headers = {
#         # Esto hace que el servidor crea que eres un usuario real
#         'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
#     }
#     try:
#         # Cambiamos HEAD por GET con stream=True para no descargar el video
#         r = requests.get(url, headers=headers, timeout=5, stream=True, allow_redirects=True)
#         # Si responde 200 o incluso si da 403/405 (a veces el stream está vivo pero bloquea el ping)
#         # lo damos por válido si el contenido parece ser de video/stream
#         return r.status_code == 200
#     except:
#         return False

# def is_link_online(url):
#     """Verifica si un link responde simulando ser un navegador (Optimizado para Telecentro/ISP)."""
#     headers = {
...
#     }
#     try:
#         # Usamos GET con stream=True (petición real pero sin descargar el video)
#         # allow_redirects=True es vital porque Telecentro suele redireccionar a otros servidores
#         r = requests.get(url, headers=headers, timeout=6, stream=True, allow_redirects=True)
        
#         # Para servidores de ISP, cualquier código entre 200 y 399 es buena señal
#         return 200 <= r.status_code < 400
#     except Exception as e:
#         # Si prefieres ver qué error da en la consola, puedes descomentar la siguiente línea:
#         # print(f" Error técnico: {e}")
#         return False

# def is_link_online(url):
#     """Verifica si un link responde simulando ser VLC (Soporte para Telecentro 206 Partial Content)."""
#     headers = {
#         'User-Agent': 'VLC/3.0.14 LibVLC/3.0.14',
#         'Accept': '*/*',
#         'Range': 'bytes=0-128', # Pedimos solo el inicio, tal como hace VLC
#         'Connection': 'keep-alive'
#     }
#     try:
#         # Usamos GET con stream=True para ser lo más parecidos a un reproductor real
#         r = requests.get(url, headers=headers, timeout=7, stream=True, allow_redirects=True)
        
#         # Aceptamos 200 (OK) y 206 (Partial Content, el que usa Telecentro)
#         if r.status_code in [200, 206]:
#             return True
            
#         # Log de depuración opcional por si quieres ver qué código da:
#         # print(f"DEBUG: {url} respondió con {r.status_code}")
#         return False
#     except Exception as e:
#         return False
