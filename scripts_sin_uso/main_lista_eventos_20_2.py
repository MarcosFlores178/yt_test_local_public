...
def get_youtube_data(channel_url, cache, original_name):
    # 1. Caché y Sistema de Respaldo (Si la señal 1 falla, busca de nuevo)
    ...
    ydl_opts_list = {
        ...
        'playlist_items': '1-20', # <--- Mantenemos tu búsqueda profunda
        'cookiefile': COOKIE_FILE,
    }

    try:
        with yt_dlp.YoutubeDL(ydl_opts_list) as ydl:
            print(f" (Escaneando 20 ítems...) ", end="")
            ...
            if 'entries' in channel_info:
                for entry in channel_info['entries']:
                    # 1. Filtro por título (Tu código viejo que funcionaba bien)
                    title = entry.get('title', '').upper()
                    if any(word in title for word in ["PROGRAMADO", "ESPERA", "PRÓXIMAMENTE", "UPCOMING"]):
                        ts = entry.get('release_timestamp')
                        if ts: proximos_ts.append(ts)
                        continue

                    # 2. Filtro por live_status estricto
                    # Solo aceptamos si YouTube dice explícitamente que es un directo ahora
                    if entry.get('live_status') != 'is_live':
                        continue 

                    # 3. Prueba de fuego: ¿Tiene link de manifiesto HLS?
                    v_id = entry.get('id')
                    link_m3u8 = extract_m3u8(f"https://www.youtube.com/watch?v={v_id}")
                    
                    # Verificamos que sea un link de manifiesto y no un video estático
                    if link_m3u8 and "yt_live_broadcast" in link_m3u8:
                        vivos_temporales.append(link_m3u8) # <--- ACÁ está la magia: no corta, suma.
